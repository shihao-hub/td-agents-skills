...
description: 飞书聊天记录归档成单文件 HTML：读取私聊/群聊区间或会话内话题（thread）的消息与图片，生成去 AI 味的纸面排版存档页（原文转录 + 编后记 + 逐张图注 + lightbox + 页内编辑）；存档 HTML 经用户确认可发回原会话（话题/区间通用），区间模式还可合并转发归档并在确认后撤回原消息，话题模式永不撤回。当用户要「归档聊天记录 / 导出飞书聊天成 html / 把聊天区间做成网页 / 把这段对话沉淀成页面 / 总结某个话题 / 转发归档后撤回」，或给出飞书消息链接要求提取聊天内容时，必须使用本 skill——即使用户只说「把昨天的自聊存一下」也要触发。不负责普通收发消息与群管理（lark-im）、审批（lark-approval）、云文档（lark-doc）。
---
