---
name: sh-codex-plugin-creator
description: 脚手架创建 Codex 插件目录、manifest 与个人市场条目，含校验和更新流程。点名使用
---

# Plugin Creator

本技能从 Codex 内置 `plugin-creator` 打包而来，提示词与脚本保持一致，脚本相对本技能根目录运行；结尾的 Codex 桌面应用深链接交接仅 Codex 内适用，其他 agent 可跳过。
...
- `references/installing-and-updating.md` for update/reinstall guidance while iterating on an existing local plugin, plus the new-thread pickup behavior after reinstall
