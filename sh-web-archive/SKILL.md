---
name: sh-web-archive
description: 把公开网页文章（公众号/博客/新闻等无需鉴权页面）一键存档为目录形态的离线三 tab 页面：原文完整转录 + AI 总结 + 可编辑页面，图片音视频全部本地化，编辑经浏览器 File System Access API 生成不可变历史版本（子 tab 倒序浏览）。当用户要「总结这个网页 / 存档这篇文章 / 把网页做成三 tab 页面 / 归档公众号文章 / 离线保存这篇教程 / 把这篇文章做成网页存档」，或给出一个 URL 要求保存/总结正文内容时，必须使用本 skill——即使用户只说「把这篇文章存一下」也要触发。只处理无需登录/Cookie 的公开页面：检测到登录墙/403/环境异常一律如实拒绝并说明，不做任何鉴权绕过；SPA（JS 渲染）页面会明确报错而非出半成品。不负责飞书文档/聊天存档（sh-lark-doc / sh-lark-chat-archive）、不负责把存档分发到消息渠道。
---
